# -*- coding: utf-8 -*-
"""解析《发酵食品挥发性物质文献数据库 (1).xlsx》为 fermented_food_db2.json。
...
SRC = "/root/uploads/1787628985967104095-发酵食品挥发性物质文献数据库 (1).xlsx"
OUT = os.path.join(HERE, "fermented_food_db2.json")
...
for r in mrows:
    ...
    if not g:
        g = dict(en=men, raw_ens=[en], cas=cas, cn=cn, cat=cat, fla=fla,
                 pending=is_pending, seen_cn=set(), prods=[])
        agg[key] = g
    else:
        g["raw_ens"].append(en)
    ...
    if cn and not is_pending:
        g["seen_cn"].add(cn)
    # 报道产品：拆分为单品后整体去重（避免"米酒/黄酒"与"米酒/黄酒、XX"重复）
    if prod:
        for p in re.split(r"[、/／,，]", prod):
            p = p.strip()
            if p and p not in g["prods"]:
                g["prods"].append(p)
...
for key, g in agg.items():
    ...
    items.append(dict(
        ...
        products="、".join(g["prods"]),    # 报道产品（来自文件主表）
